一、实践背景  
本实践基于 MIT xv6（x86 版本），围绕操作系统从 bootloader 到内核 main 函数的完整启动流程展开，配合视频学习 bootblock、bootasm.S、bootmain.c、entry.S 以及内核页表初始化过程。  
二、实践目标  
...
3.任务2：启动流程注释  
详情可以看具体文档bootasm.S、bootmain.c、entry.S  
4.任务3：启动过程可视化（核心任务）  
(1)初始尝试：VSCode 日志断点  
